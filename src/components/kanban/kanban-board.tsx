...
export function KanbanBoard({ customers, onCustomersUpdated, dateFilter }: KanbanBoardProps) {
  const [columns, setColumns] = useState<Record<string, Customer[]>>({
    "Neu": [], // "Yeni" -> "Neu"
    "Anzurufen": [], // "Aranacak" -> "Anzurufen"
    "Rausgefallen": [],
    "Rausgefallen WP": [],
  });
  ...
  useEffect(() => {
    ...
    filteredCustomers.forEach(customer => {
      if (newColumns[customer.qc_on]) {
        newColumns[customer.qc_on].push(customer);
      } else {
        // Wenn der qc_on Wert keine gültige Spalte ist, füge ihn der Spalte "Neu" hinzu
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      await updateCustomer(draggableId, { qc_on: destination.droppableId });
